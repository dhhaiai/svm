...
public class svm_scale
{
	...
	private String ret_filename = null;
	...
	private void run(String []argv) throws IOException
	{
		...
		for(i=0;i<argv.length;i++)
		{
			...
			switch(argv[i-1].charAt(1))
			{
				...
				case '>': ret_filename = argv[i]; break;
				default:
		...
		if(ret_filename != null && !ret_filename.equals(""))
		{
			File ret_file = new File(ret_filename);
			if(!ret_file.exists())
			{
				ret_file.createNewFile();
			}
			writer = new BufferedWriter(new FileWriter(ret_file));
		}
